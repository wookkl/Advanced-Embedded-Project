...
unsigned char left[3] = {1,1,30};
unsigned char right[3] = {1,0,30};
unsigned char stop[3] = {0,0,0};

int EXIT;
int status;
int client_sockfd,client_len,frame_fd;
int dev_button,dev_fnd,dev_dot,dev_led,dev_lcd,dev_motor,dev_us;
int available;
int count_car;
unsigned char led;
int* pfbdata;
...
void* t_recv_from_server(){
	...
	int rcv_byte,i;
	while(1){
		memset(msg,0,sizeof(char)*BUFF_SIZE);
...
void Charge(char* info){
	...
	if(area==-1){
		...
		sprintf(str1,"NUM_%s's fee: ",car_number);
		sprintf(str2,"%s  byebye~",fee);
		Print_lcd(str1,str2);
		Lcd_clear();
	}
	free(fee);
...
void Exit_button(){
	send(client_sockfd,"exit",strlen("exit")+1,0);
	sleep(1);
	EXIT=TRUE;
}
...
void MotorLeft(){
	write(dev_motor, left,3); 
}
void MotorStop(){ 
	write(dev_motor, stop,3); 
}
void MotorRight(){
	write(dev_motor, right, 3);
}
int Makepixel(int  r, int g, int b){
